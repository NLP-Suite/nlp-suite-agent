...
def df_to_csv(data_frame, outputFilename, headers=None, index=False, language_encoding='utf-8'):
    while True: # repeat until file is closed
        try:
            ...
            print('Output file error', "Could not write the file " +
...
def list_to_csv(list_output,outputFilename,colnum=0, encoding='utf-8'):
    error = False
...
def export_csv_to_text(inputFilename, outputDir, column=None, column_list=[]):
    ...
    if column != None and len(column_list) != 0:
        print('Field(s) input error', 'Cannot have field and field_list as filter at the same time.\n\nPlease, select one and try again.')
        return
    if column == None and len(column_list) == 0:
