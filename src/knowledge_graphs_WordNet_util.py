...
def disaggregate_GoingDOWN(WordNetDir,outputDir, wordNet_keyword_list, noun_verb):
    ...
    startTime=IO_user_interface_util.timed_alert(4000, 'Analysis start', 'Started running WordNet (Zoom IN/DOWN) at', True, 'Running WordNet with the ' + noun_verb + ' option with following keywords:\n\n' + str(wordNet_keyword_list))
    warning = subprocess.call(call_list)
    if warning == 1:
        IO_user_interface_util.timed_alert(3000, 'Warning',
    ...
    IO_user_interface_util.timed_alert(2000,'Analysis end', 'Finished running WordNet (Zoom IN/DOWN) at', True, '', True, startTime)
    return filesToOpen
...
def aggregate_GoingUP(WordNetDir, inputFile, outputDir, config_filename, noun_verb,openOutputFiles,chartPackage, dataTransformation, language_var=''):
    ...
    if errorFound:
        return filesToOpen
    startTime=IO_user_interface_util.timed_alert(4000, 'Analysis start', 'Started running WordNet (Zoom OUT/UP) with the ' + noun_verb + ' option at', True, '',True,'',True)
    ...
    if warning == 1:
        IO_user_interface_util.timed_alert(3000, 'Invalid Input',
    ...
    IO_user_interface_util.timed_alert(2000,'Analysis end', 'Finished running WordNet (Zoom OUT/UP) at', True, '', True, startTime, True)
