...
# Ngrams searches & VIEWER','Word searches
    ...
    if search_token_var.get() and searchField_kw=='e.g.: father':
        raise(Exception('No search token entered'))
...
# ANALYSES -----------------------------------------------------------------------------------
    ...
    if all_analyses_var.get():
        ...
        startTime=IO_user_interface_util.timed_alert(2000,'Analysis start', 'Started running CoNLL table ' + label + ' analyses at',
                                                     True, '', True, '', False)
...
# SEARCH -----------------------------------------------------------------------------------
    ...
    if search_token_var.get() and searchField_kw != 'e.g.: father':
        ...
        if ' ' in searchField_kw:
            raise Exception("Search error, the CoNLL table search can only contain one token/word")
            # mb.showwarning(title='Search error',
        ...
        if 'e.g.: father' in searchField_kw:
            if (not os.path.isfile(inputFilename.strip())) and \
                ...
                raise(Exception("INPUT File Path Error"))
            ...
            raise(Exception("Searched Token Input Error"))
        ...
        if len(data) <= 1000000:
            try:
                ...
                raise (Exception("The CoNLL table is ill formed. You may have tinkered with it. Please, rerun the Stanford CoreNLP parser since many scripts rely on the CoNLL table."))
...
# -----------------------------------------------------------------------------------------------------------------------------
    if k_sentences_var.get():
        if Begin_K_sent_var==0 or End_K_sent_var==0:
            print("The Repetion finder algorithm needs beginning and end K sentences.\n\nPlease, enter valid K number(s) of sentences and try again.")
