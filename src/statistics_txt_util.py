...
def compute_corpus_statistics( inputFilename, inputDir, outputDir, configFileName, openOutputFiles, chartPackage, dataTransformation,
    ...
    outputFiles = statistics_csv_util.compute_csv_column_statistics_NoGroupBy( outputFilename, outputDir, False,
...
def compute_line_length( configFileName, inputFilename, inputDir, outputDir,openOutputFiles,chartPackage, dataTransformation):
    filesToOpen=[]
    ...
    if IO_csv_util.openCSVOutputFile(outputFilename):
        return
    startTime=IO_user_interface_util.timed_alert(2000,'Analysis start', 'Started running line length analysis at',
                                                 True, '', True, '', True)
    ...
    IO_user_interface_util.timed_alert(2000,'Analysis end', 'Finished running line length analysis at', True, '', True, startTime, True)
...
def compute_character_word_ngrams(inputFilename,inputDir,outputDir, configFileName,
    ...
    if inputFilename=='' and inputDir=='':
        print('Input error', 'No input file or input directory have been specified.\n\nThe function will exit.\n\nPlease, enter the required input options and try again.')
        return

    startTime = IO_user_interface_util.timed_alert(3000, 'N-Grams start',
...
def yule( inputFilename, inputDir, outputDir, configFileName, hideMessage=False):
    ...
    for doc in inputDocs:
        ...
        if inputFilename!='' and hideMessage==False:
            IO_user_interface_util.timed_alert(4000, message_title='Yule’s K Vocabulary richness', message_text='The value for the vocabulary richness statistics (word type/token ratio or Yule’s K) is: '+str(result) + '\n\nValue range: 0-100. The higher the value, the richer the vocabulary.')
...
def print_results( words, class_word_list, header, inputFilename, outputDir, excludestowords, fileLabel, hideMessage, filesToOpen):
    if excludestowords:
    ...
    if hideMessage==False:
        # do not count header
        print('Results', 'Total word count ' + stopMsg + ': ' + str(len(words)) +
...
def process_words( configFileName, inputFilename,inputDir,outputDir, openOutputFiles, chartPackage,dataTransformation,
    ...
    if processType == '' or "N-grams" in processType or \
        ...
        wordgram = True
        bySentenceID = False
        outputFiles, tempoutputDir = compute_character_word_ngrams( inputFilename, inputDir, outputDir, configFileName,
                                                        ngramsNumber, frequency, hapax_words,
                                                        normalize,
...
# WORD LENGTH --------------------------------------------------------------------------
    ...
    if len(word_list)==0:
        IO_user_interface_util.timed_alert(2000,"Empty file",
...
def convert_txt_file(inputFilename,inputDir,outputDir,openOutputFiles,excludeStopWords=True,lemmatizeWords=True):
    ...
    inputDocs=IO_files_util.getFileList(inputFilename, inputDir, fileType='.txt', silent=False, configFileName=GUI_IO_util.configFileName)

    Ndocs=str(len(inputDocs))

    startTime=IO_user_interface_util.timed_alert(2000,'Analysis start', 'Started running txt conversion (lemmatization & stopwords) at',
                                                 True, '', True, '', True)
...
def compute_sentence_text_readability( inputFilename, inputDir, outputDir, configFileName, openOutputFiles, chartPackage, dataTransformation):
    filesToOpen = []
    documentID = 0
    ...
    IO_user_interface_util.timed_alert(2000,'Analysis end', 'Finished running Text Readability at',
                                       True, '', True, startTime)
...
def compute_sentence_complexity( inputFilename, inputDir, outputDir, configFileName, openOutputFiles, chartPackage, dataTransformation):
    ## list for csv file
    columns=[]
    documentID = []
    document = []
