...
def visualize_chart_byGroup(
    ...
    columns_to_be_plotted_byGroup = [
        [docCol, groupBy_Field]
    ]  # will give different bars for each value
...
def visualize_chart_bySent(
    ...
    if (
        count_var == 1
    ):  # for alphabetic fields that need to be counted for display in a chart
        temp_outputFilename = statistics_csv_util.compute_csv_column_frequencies(
            inputFilename,
            None,
            outputDir,
        ...
        if pivot:
            inputFilename = statistics_csv_util.csv_data_pivot(
                temp_outputFilename[0], "Sentence ID", "Gender", no_hyperlinks=True
            )
        else:
        ...
        if n_documents > 1:
            for i in range(0, len(columns_to_be_plotted_numeric)):
                ...
                columns_to_be_plotted_bySent.append(
                    [docCol + 1, columns_to_be_plotted_numeric[i][0]]
                )
        else:
            # Sentence ID, Frequency
            columns_to_be_plotted_bySent.append(
                [sentCol, columns_to_be_plotted_numeric[i][0]]
            )
...
def visualize_chart(
    ...
    bySent = False
    if bySent:
        fileToOpen = visualize_chart_bySent(
            inputFilename,
            outputDir,
            chartPackage,
            filesToOpen,
...
def run_all(
    ...
    if data_to_be_plotted == None:
        print("Data to be plotted was none!")
        return
    ...
    if data_to_be_plotted == None:
        print("Data to be plotted was none x2!")
        return
    else:
...
def Sankey(
    ...
    if not pd.api.types.is_string_dtype(data[var1]) or not pd.api.types.is_string_dtype(data[var2]):
        print(
            "Warning: All csv file fields should be categorical for a Sankey flowchart.\nPlease, select categorical fields with string values and try again."
        )
        return
    ...
    if three_way_Sankey:
        ...
        sourcelist = list(range(len(finalframe[var1].unique())))
        source = [s for s in sourcelist for _ in range(len(finalframe[var2].unique()))]

        target1 = list(range(len(finalframe[var2].unique())))
        target2 = [t + len(finalframe[var1].unique()) for t in target1]
        target = target2 * len(finalframe[var1].unique())

        valuevector = []
        for i in finalframe[var1].unique():
            tempdata = finalframe[finalframe[var1] == i][var2].value_counts()
            for j in finalframe[var2].unique():
                valuevector.append(tempdata.get(j, 0))
...
def Sunburst(
    ...
    if (first_sentences == None and last_sentences != None) or (
        first_sentences != None and last_sentences == None
    ):
