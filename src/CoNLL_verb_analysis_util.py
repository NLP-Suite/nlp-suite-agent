...
def verb_voice_stats(inputFilename, outputDir, data, data_divided_sents, openOutputFiles, chartPackage, dataTransformation):
	...
	IO_csv_util.df_to_csv(df, verb_voice_file_name, headers=headers, index=False,
...
def verb_modality_stats(config_filename, inputFilename, outputDir, data, data_divided_sents, openOutputFiles,
	...
	IO_csv_util.df_to_csv(df, verb_modality_file_name, headers=headers, index=False,
						  language_encoding='utf-8')
	...
	if chartPackage!='No charts':
		...
		IO_csv_util.df_to_csv(df, verb_modality_value_stats_file_name, headers=headers, index=False,
							  language_encoding='utf-8')
...
def verb_compute_frequencies(inputFilename, outputDir, data, data_divided_sents, openOutputFiles, chartPackage, dataTransformation):
	...
	IO_csv_util.df_to_csv(df, verb_file_name, headers=['Form', 'Lemma'], index=False,
						  language_encoding='utf-8')
	...
	IO_csv_util.df_to_csv(merged_df, verb_stats_file_name, headers=['Form', 'Form Frequency', 'Lemma', 'Lemma Frequency'], index=False,
						  language_encoding='utf-8')
...
def verb_tense_stats(inputFilename, outputDir, data, data_divided_sents, openOutputFiles, chartPackage, dataTransformation):
	...
	IO_csv_util.df_to_csv(df, verb_tense_file_name, headers=headers, index=False,
						  language_encoding='utf-8')
...
def verb_stats(config_filename, inputFilename, outputDir, data, data_divided_sents, openOutputFiles, chartPackage, dataTransformation):
	...
	IO_user_interface_util.timed_alert('Analysis end', 'Finished running VERB ANALYSES at', True,
									   '', True, startTime, True)
